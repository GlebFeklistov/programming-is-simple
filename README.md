...
### Комментарии
> Комментарии - пояснения к исходному тексту программы, находящиеся непосредственно внутри исходного кода программы. Комментарии разделяются на однострочные и многострочные. Однострочный комментарий отмечается специальным символом в начале (например, //) и продолжается до конца строки. Многострочный комментарий может иметь любую длину, он отмечается специальными символами в начале и конце (например, /* \*/).

Синтаксис комментариев определяется языком программирования. Эти части текста программы, не влияют на её семантику. Комментарии не оказывают никакого влияния на результат выполнения программы. Помимо исходных текстов программ, комментарии также применяются в языках разметки и языках описания.

Пример кода содержащего комментарии:

```js
/**
 * Это многострочные комментарий
 *
 * Здесь можно описать работу данного кода
 * или просто оставить памятку для себя
 */
var pointFactory = function (x, y) {
  return { x, y } // Это однострочный комментарий, здесь перенос текста недопустим
}
```

:books: [Примеры](/comments.md)
...
greet('World'); // результатом работы этой функции будет вывод к консоль "Hello World!"
```
...
console.log(a); // выведет undefined, т.к. переменной не присвоено значение
console.log(bool); // выведет "true"
console.log(char); // выведет "z"
```
...
Пример объявления переменных составных типов:
...
```js
let pi = 3.141592653589793; // объявлена переменная и сразу ей присвоено значение
let user; // переменная объявлена, но ей не присвоено значение, она пустая
```
...
![Пример поведения любой программы](/basic_flow.png)
...
Пример вывода в консоль фразы "Hello World!":
...
Пример различных выражений:

```js
var a = 'Hello ' + 'World!'; // в a теперь хранится "Hello World!"
var b = true && false // в b теперь хранится false
var с = 10 + 5; // в с теперь хранится 15
```
...
```js
var a = 10; // объявление переменной и присвоения начального значения 10

a += 3; // в развернутом виде a = a + 3
a -= 5; // в развернутом виде a = a - 5
```

:books: [Примеры](/assignment.md)
...
```js
var a = 10 + 5; // результат 15
var b = 10 * 5; // результат 50
var c = 10 / 5; // результат 2
var d = 12 % 5; // результат 2
```

:books: [Примеры](/math.md)
...
```js
var a = 'Hello' == 'World!'; // результат false
var b = 10 > 5; // результат true
var c = 10 != 5; // результат true
var d = 10 <= 5; // результат false
var e = 'a' == 'b'; // результат false
```

:books: [Примеры](/relations.md)
...
```js
var a = true && true // результат true
var b = true && false // результат false
var c = false && true // результат false
var d = true  || true // результат true
var e = false || true // результат true
var f = true  || false // результат true
var g = false || false // результат false
```

:books: [Примеры](/logic.md)
...
```js
var a = 'John' + 'Doe'; // рузультат "JohnDoe", все будет слитно
var b = 'Hello' + ' ' + 'World!'; // рузультат "Hello World!", тут уже есть отступ
var c = 'Hello ' + 'John Doe' ; // рузультат "Hello John Doe"
var d = 'Hello' == 'World!'; // результат false
var e = 'a' == 'b'; // результат false
```

:books: [Примеры](/strings.md)
...
Пример который проверяет утверждение, что 5 больше 10, в случае выполнения этого условия выведет в консоль сообщение "5 больше 10", в противном случае выведет сообщение "5 меньше 10":
...
Пример который проверяет утверждение, что значение переменной 5 больше или равно 10, в случае выполнения этого условия в консоль будет выведен текст, в противном случае программа пропусти набор команд и продолжит своё выполнение:
...
Пример который демонстрирует работу переключателя, здесь предусмотрены три логические ветви, в первом случае если в переменную "creature" присвоено значение 'Волк' или 'Заяц' будет выведена в консоль строка содержащая имя этого персонажа, во всех остальных случаях будет выведена строка из ветки 'default':
...
switch (creature) {
  ...
  case 'Заяц':
    ...
    break;
  default:
    console.log('Другой сказочный персонаж питающий слабость к капусте');
    break;
}
```
...
Пример который запускает пять итераций цикла и на каждой выводит в консоль значение счетчика:
...
Пример который запускает восемь итераций цикла и на каждой выводит в консоль значение счетчика:
...
Пример который запускает три итерации цикла и на каждой выводит в консоль значение счетчика:
...
В большинстве языков, цикл for, несмотря на синтаксическую форму, в действительности является циклом с предусловием, но имеет более удобную в использовании форму записи и изоляцию переменной используемой в качестве счетчика:

Пример который демонстрирует семантические различия в разных типах циклов, которые при этом полностью тождественны по условиям:
...
// запись выше полностью эквивалентна записи цикла ниже
...
```js
var utils = require('./myFirstModule'); // подключение модуля
