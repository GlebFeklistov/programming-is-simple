...
В большинстве языков, цикл for, несмотря на синтаксическую форму цикла со счётчиком, в действительности является циклом с предусловием, но более удобна в использовании и изоляции переменной используемой в итерации цикла, таким образом код выще полностью повторяет функциональность кода который ниже:
...
##### Пропуск итерации
Существую случаи когда в текущей итерации цикла необходимо пропустить все команды до конца тела цикла. При этом сам цикл прерываться не должен, условия продолжения или выхода должны вычисляться обычным образом, для такого случаю предусмотрена команда пропуска итерации, обычно это **continue**.
