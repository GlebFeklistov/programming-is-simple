...
```js
var i = 0;  //  Здесь происходит объявление переменной для счетчика итераций
while (i < 3) { //  Здесь проверяется условие по которому пока i < 3 будет выполняться тело цикла
  alert(i); //  Какой-то полезный код
  i++;  //  Здесь происходит увеличение счетчика на 1
}
```
...
```js
for (var i = 0; i < 3; i++) { //  Здесь происходит объявление переменной для счетчика итераций, проверка условия и определение правил изменения счетчика при каждой итерации
  alert(i); //  Какой-то полезный код
}
```
...
```
function max(a, b) {  //  Здесь происходит объявление функции, ей присваивается имя max и в качестве входных параметров a, b
  return a > b ? a : b; //  это тело функции в котором происходит выбор наибольшего числа из двух входящих и возвращается наибольшее
}
```
