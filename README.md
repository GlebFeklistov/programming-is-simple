...
На данной странице будут демонстрироваться примеры кода написанные на JavaScript (NodeJS), примеры на других языках будут доступны по ссылкам под примерами вместе с коментариями к самому коду.
...
В целях еще большего упрощения семантического анализа, рекомендуется документировать участки кода сложные для понимания или выполняющие специфические функции. Данная документация может быть легко преобразована в электронную документацию по вашему коду с помощью определенных решений, которые есть для каждого языка.
...
Пример присвоения литерала переменной "Cat" и вывода литерала "Мой кот вышел погулять" в консоль:
```js
...
Пример объявления и вызова функции на JavaScript:
```js
function greet(user) {
