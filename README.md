...
Конструкция if-else имеет семантическое значение в переводе с английского if - если, a else - иначеж. Что является очень удобным при изучении языков программирования, так как вся такая конструкция читается как обычный текст:

if (условине) { блок1 } else { блок2 } - если условие истинно то выполнить блок1 иначе выполнить блок2
